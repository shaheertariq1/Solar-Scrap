...
export default function SellersPage() {
  ...
  useEffect(() => {
    ...
    getAdminUsers("seller")
      ...
      .catch((err) => console.error("Error loading sellers from API:", err))
  ...
  const handleRejectUser = async (user: SellerAccount) => {
    try {
      await updateUserStatus(user.id, "rejected");
      setSellers((prev) =>
        prev.map((s) =>
          s.id === user.id ? { ...s, status: "Rejected", activityStatus: "Inactive" } : s
        )
      );
  ...
  return (
    <div className="flex h-screen w-full bg-[#F5F6FA] overflow-hidden">
          ...
          <main className="flex-1 p-5 sm:p-7 md:p-8 space-y-6 overflow-y-auto">
            ...
            <div className="bg-white rounded-2xl border border-gray-200/80 shadow-xs overflow-hidden">
                          ...
                          <div className="flex items-center gap-2.5">
                            {seller.profilePhotoUrl ? (
                              <div className="relative w-7 h-7 rounded-full overflow-hidden shrink-0 ring-1 ring-gray-200">
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img
                                  src={getAvatarUrl(seller.profilePhotoUrl)!}
                                  alt={seller.name}
                                  className="w-full h-full object-cover"
                                />
                              </div>
                          ...
                          {actionMenuOpenId === seller.id && menuPos && (
                            <>
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleRejectUser(seller)}
                      ...
                      <tr>
                        <td colSpan={7} className="py-12 text-center text-xs text-gray-500 font-medium">
                          <div className="inline-flex items-center gap-2">
                            ...
                            <span>Loading sellers from database...</span>
                          </div>
                        </td>
                      </tr>
              ...
              <div className="px-5 sm:px-6 py-3.5 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500 bg-white">
                <span>
                  Showing {filteredSellers.length} of {sellers.length} records
                </span>
      ...
      {/* ===================== 1. SELLER DETAILS MODAL (Figma Design) ===================== */}
